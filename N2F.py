...
if __name__ == "__main__":
    ...
    # for v in range(1,2):
        ...
        while notdone:     
            ...
            img = loadmat(folder + '/' + file_name)['noisy'].astype(np.float32) #/255 for SIDD images
            # print(img1)
            # img2=np.zeros(img1.shape,np.float32)
            # img2[:,:,0] = img1[:,:,2]
            # img2[:,:,1] = img1[:,:,1]
            # img2[:,:,2] = img1[:,:,0] # converting rgb to bgr
            # ycbcr=cv2.cvtColor(img2, cv2.COLOR_BGR2YCrCb)
            # img=ycbcr[:,:,0]
            print(img.shape)
            typer = type(img[0,0])
            ...
            if np.sum(np.round(H[1:-1,1:-1]-np.mean(H[1:-1,1:-1]))>0) <= 25 and learning_rate != 0.000005:
                learning_rate = 0.000005 #original line
                # learning_rate /=2 #my logic of decreasing lr
        ...
        # out[:,:,1]= ycbcr[:,:,1]
        # out[:,:,2]= ycbcr[:,:,2]
        # out = cv2.merge((H, ycbcr[:,:,1], ycbcr[:,:,2] ))
        # out = cv2.cvtColor(out, cv2.COLOR_YCrCb2RGB)
        # imwrite(outfolder + '/' + file_name, np.round(H).astype(typer))
        savemat(outfolder + '/' + file_name.split('.')[0]+"-N2f.mat",{"Denoised": H.astype(np.float32)})
