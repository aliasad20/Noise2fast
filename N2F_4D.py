...
if __name__ == "__main__":
    ...
    for v in range(len(file_list)):
        ...
        inp = loadmat(folder + '/' + file_name)['noisy'].astype(np.float32)
        if inp.shape[-1] == 3:
        ...
        try:
            ...
            savemat(outfolder + '/' + file_name,{"Denoised": np.round(out).astype(typer)})
        except:
            # imwrite(outfolder + '/' + file_name, np.round(out).astype(np.float32), imagej=True)
            savemat(outfolder + '/' + file_name,{"Denoised": np.round(out).astype(np.float32)})
